...
#         mono_bin_df = base_df.select(*mono_bin_list)
#         mono_bin_df = mono_bin_df.select([f.col(c).cast(IntegerType()) for c in mono_bin_df.columns])
#         mono_bin_iv_woe = mono_bin(mono_bin_df)
#     else:
#         char_bin_list.append(i)
#         char_bin_df = base_df.select(*char_bin_list)
#         char_bin_df = char_bin_df.select([f.col(c).cast(StringType()) for c in char_bin_df.columns])
#         char_bin_iv_woe = char_bin(char_bin_df, char_bin_df[target])
# 
# 
# IV_WOE_df = char_bin_iv_woe.union(mono_bin_iv_woe).orderBy(f.col('var_name'),f.col('MIN_VALUE'))
# IV_WOE_df.show()
...
# 
# 
# # # CHAR BIN IMPLEMENTATION COMPLETED
# # #char_bin_iv_woe.show()
# # mono_bin_df = unpivot_data(mono_bin_df)
# # mono_bin_df = mono_bin_df.withColumn('bucket',f.ntile(MAX_BIN).over(Window.partitionBy(f.col('var_name')).orderBy(f.col('values'))))
# # mono_bin_df_grouped = mono_bin_df.groupby(f.col('var_name'),f.col('bucket'))
# #
# # #MAX, MIN and COUNT
# # mono_cal_df = mono_bin_df_grouped.agg(f.min(f.col('values')).alias('MIN_VALUE')
...
# #                          .withColumn('DIST_NON_EVENT', (f.col('NON_EVENT')/nonEVENT_sum).cast(FloatType()))
# #
# # mono_cal_df = mono_cal_df.withColumn('WOE', (f.log(f.col('DIST_EVENT')/f.col('DIST_NON_EVENT'))).cast(FloatType()))
# # mono_cal_df = mono_cal_df.withColumn('IV', ((f.col('DIST_EVENT') - f.col('DIST_NON_EVENT')) * f.col('WOE')).cast(FloatType())).drop('bucket')
# #
# # mono_cal_df.show()
# # #mono_cal_df.show()
# # #char_bin_iv_woe.show()
...
# 
# 
# # char_bin_unstacked_df = unpivot_data(char_bin_df)
# #
# # sum_EVENT_nonEVENT = char_bin_unstacked_df.select(f.col(target)).groupBy(char_bin_unstacked_df[target]).count().withColumnRenamed \
# #         (target, 'targetValues').withColumnRenamed('count', 'sum_value')
# # EVENT_sum = sum_EVENT_nonEVENT.where(sum_EVENT_nonEVENT['targetValues'] == 1).select('sum_value').first()[0]
# # nonEVENT_sum = sum_EVENT_nonEVENT.where(sum_EVENT_nonEVENT['targetValues'] == 0).select('sum_value').first()[0]
# 
# 
# #EVENT, and NON EVENT
# #mono_bin_df_event = mono_bin_df_grouped.sum()
# #mono_bin_df_event.show()
# 
# 
# #mono_bin_unstacked_df = mono_bin_df_grouped.withColumn('MIN_VALUE', mono_bin_df_grouped.groupby(f.col('var_name'),f.col('bucket')).min())
